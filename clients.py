...
class client(object):
    ...
    def localUpdate(self, localEpoch, localBatchSize, Net, lossFun, opti, global_parameters,lossList):
        self.lossNum = 0
        ...
        for epoch in range(localEpoch):
            for data, label in self.train_dl:
                ...
                if(self.lossNum<=500):
                    lossList.append(loss.item())#记录loss数据并返回
                loss.backward()
                opti.step()
                opti.zero_grad()
...
class ClientsGroup(object):
    ...
    def dataSetBalanceAllocation(self):
        ...
        self.test_data=mnistDataSet.test_data
        self.test_label=mnistDataSet.test_label
        self.test_data_size=mnistDataSet.test_data_size
